...
// Depth-First Search function with depth tracking
bool dfs(State currentState, set<vector<vector<int>>>& visited, int depth) {
    ...
    for (const State& nextState : currentState.successors()) {
        ...
        if (visited.find(nextState.board) == visited.end()) {
            ...
            cout << "-----------------" << endl;
            // Recursively call dfs on the successor state with an increased depth level
...
if (!dfs(initialState, visited, 1)) // Call the dfs function with initial state, passing the set of visited states and starting depth level 0
    cout << "No solution found!" << endl; // Print "No solution found!" if dfs function returns false
